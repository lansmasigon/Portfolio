...
import profileImage from '../assets/Masigon_LanceGabriel.png';
import * as THREE from 'three';
import './Lanyard.css';

extend({ MeshLineGeometry, MeshLineMaterial });

export default function Lanyard({ position = [0, 0, 15], gravity = [0, -40, 0], fov = 45, transparent = true }) {
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }) {
  ...
  // Configure profile texture
  useEffect(() => {
    if (profileTexture) {
      profileTexture.center.set(0.45, 0.45);
      profileTexture.rotation = Math.PI; // Rotate 180 degrees to fix upside down
      profileTexture.offset.set(-0.25, 0); // Move image to the left to be fully visible
      profileTexture.repeat.set(1.5, 1.5); // Scale up to show full image - bigger zoom
    ...
    <>
      <group position={[0, 8, 0]}>
        ...
        <RigidBody position={[3.2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[1.8, 2.5, 0.01]} />
          <group
            scale={5}
            position={[0, -1.2, -0.05]}
